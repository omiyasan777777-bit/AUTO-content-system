...
export interface CompatibilityReading {
  ...
  /** 2人の数を足して還元した縁の数 */
  pairNumber: number;
  /** 相性の質を表すタイトル（例: 静かに深まる縁） */
  title: string;
  body: string;
}

/** 相手の生年月日が無い場合は自分の恋愛気質を深掘り */
...
export type BondReading = CompatibilityReading | SelfReading;
...
export interface PremiumFortuneResult extends FortuneResult {
  cards: DrawnCards;
  bond: BondReading;
  timeline: [TimelineEntry, TimelineEntry, TimelineEntry];
  lucky: LuckyCharm;
}

/** 履歴に保存するエントリ（端末のlocalStorage内のみ・外部送信なし） */

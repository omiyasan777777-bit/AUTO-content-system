...
export function DistributorLinks() {
  ...
  return (
    <section
      aria-label="監修者のご案内"
      className="rounded-2xl border border-gold-soft bg-white/80 p-5 text-center"
    >
      <p className="text-xs tracking-widest text-gold">SUPERVISED BY</p>
      <p className="mt-1 font-serif text-lg text-ink">{distributor.name}</p>
      {distributor.bio && (
      ...
      <div className="mt-4 flex flex-col gap-2">
        {links.map(({ url, label, icon: Icon }) => (
          <a
            ...
            className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-mauve-300 bg-white px-6 text-sm font-medium text-mauve-700 transition-colors hover:bg-mauve-100"
          >

...
1. このフォルダをデスクトップ等に配置
2. Windows: `start_webapp.bat` をダブルクリック / Mac・Linux: `start_webapp.sh` を実行
   - 初回は必要なパッケージ（flask, selenium 等）を自動インストールしてから起動する
   - ブラウザが自動で開き（http://127.0.0.1:8787）、ボタン・プルダウンで操作できる画面が表示される
3. 画面の「⚙ セットアップ」からnote.comにログイン → メインメニューから制作開始

## セットアップ（Claude Codeのチャットで直接使う場合）
